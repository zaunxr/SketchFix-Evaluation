...
# ---------------------------------------------------------------------
def repalceLibPath(projPath_i, configDict, jarRootPath, proj):
...
# ---------------------------------------------------------------------
# TODO Create SKETCHES WHERE NO Sketch is deployed.
# ---------------------------------------------------------------------
def createPatches(projPath_i):
    # Fetch the patchGenerator and put it in the java src.
    patchGenerator_src = os.path.join(jarRootPath, "PatchGenerator", "PatchGenerationRunner.java")
    patchGenerator_dst = projPath_i + "/src/main/java/"
    patchGenerator_dst = patchGenerator_dst + "PatchGenerationRunner.java"
    shutil.copyfile(patchGenerator_src, patchGenerator_dst)

    # Fetch the config.txt file and put it into the main project folder
    configFile_src = os.path.join(jarRootPath, "PatchGenerator", "PatchGenerationConfig.txt")
    configFile_dst = projPath_i + "/PatchGenerationConfig.txt"
    shutil.copyfile(configFile_src, configFile_dst)
    print("The PatchGenerator is inserted manually - Now sketches could be created manually.")
...
def testAllPatches(projPath_i, configDict, sketchesPath):
    ...
    if len(patchList) == 0:
        print("✖ No sketches are found. The PatchGenerator manually is inserted in the buildPath.")
        createPatches(projPath_i)
...
def saveResult(configDict, patchDict, resultPath_i, sketchesPath_i):
    ...
    with open(os.path.join(resultPath_i, "results.txt"), 'w+') as file:
        file.write("##################################################################### \n")
        file.write("				       RESULTS FOR " + currentProj)
        file.write("\n")
        file.write("\n #####################################################################")
        file.write("\n Total Plausible patches: " + str(total_sketches_valid))
        file.write("\n Total time for Plausible patches in seconds: " + str(total_delta_time_valid))
        file.write("\n Total time for Plausible patches in seconds: " + str(total_delta_time_valid / 60))
    ...
    for patch_i in patchDict["vpList"]:
    ...
    # Return the time
    return total_sketches_valid, total_sketches_nonvalid, total_delta_time_valid, total_delta_time_nonvalid
...
projDict["Closure"] = [62, 126]
# Closure 14 has no build.xml file
projDict["Lang"] = [6, 55, 59]
projDict["Math"] = [5, 33, 50, 59, 70, 82, 85]
projList = ["Chart", "Math", "Lang", "Closure"]
...
sketches_All_valid = 0
sketches_All_invalid = 0
time_valid_All = 0
time_nonvalid_All = 0
for proj_i in projList:
    ...
    print("##################################################")
    sketches_proj_invalid = 0
    sketches_proj_valid = 0
    time_valid_proj = 0
    ...
    for ver_j in projDict[proj_i]:
        ...
        print("➳ SketchFix Plausible/Invalid Result (resultPath_i):")
        print(resultPath_i)
        ...
        repalceLibPath(projPath_i, configDict, jarRootPath, proj_i)
        ...
        total_sketches_valid, total_sketches_nonvalid, total_time_valid, total_time_nonvalid = saveResult(configDict, patchDict,
                                                                                 resultPath_i + currentProjFolder,
                                                                                 sketchesPath_i)
        ...
        print("☐ ☐ Time needed for :", projectname + "_" + version, (total_time_valid + total_time_nonvalid) / 60,
              "minutes.")
        sketches_proj_valid = sketches_proj_valid + total_sketches_valid
        sketches_proj_invalid = sketches_proj_invalid + total_sketches_nonvalid
    ...
    print("☐ ☐ ☐ Sketches Plausible:", sketches_proj_valid)
    print("☐ ☐ ☐ Sketches invalid:", sketches_proj_invalid)
    print("                                                  ")
    print("++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("                                                  ")
    ...
    with open(os.path.join(resultPath_i, "results.txt"), 'w+') as file:
        file.write("##################################################################### \n")
        file.write("				   RESULTS	FOR " + projectname)
        file.write("\n")
        file.write("\n #####################################################################")
        file.write("\n Total Plausible patches: " + str(sketches_All_valid) + " \n Invalid patches: "+ str(sketches_All_invalid))
        file.write("\n Total time for patches in seconds: " + str(time_valid_proj + time_nonvalid_proj))
        file.write("\n Total time for patches in minutes: " + str((time_valid_proj + time_nonvalid_proj) / 60))

    sketches_All_valid = sketches_All_valid + sketches_proj_valid
    sketches_All_invalid = sketches_All_invalid = sketches_proj_invalid
...
print("☑ Plausible Sketches:" + str(sketches_All_valid))
print("☑ Invalid Sketches:" + str(sketches_All_invalid))
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------